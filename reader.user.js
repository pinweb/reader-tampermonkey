...
(function() {
    ...
    function genNewUrl(oldUrl){
        ...
        return 'http://pdf.pinweb.io/n/'+safeUrl(CryptoJS.AES.encrypt(JSON.stringify(oldUrl), key, {mode: CryptoJS.mode.ECB}).toString());
    }
    ...
    function addReaderLink() {
        ...
        for(var i=0;i<as.length;i++){
            ...
            if(as[i].href.startsWith('http://read.tjad.cn/')){
                ...
                as[i].parentElement.insertAdjacentHTML('beforeend','<a href="'+newUrl+'" target="_blank" class="ml-4 text-red-600">测试新版阅读</a>')
            }else{
    ...
    function parseLink(link){
        return PHPUnserialize.unserialize(atob(link.replace('http://read.tjad.cn/t/','')));
    }
